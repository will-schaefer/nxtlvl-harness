...
| Contract | Definition |
|---|---|
| **Managed TOML region** | A marker-delimited block (`# --- nxtlvl-managed BEGIN/END ---`) the compiler owns inside a CLI-owned file. Inserted **before the first `[table]` header** (TOML top-level keys must precede tables); replaced in place on re-run; idempotent (second run is a byte no-op). Everything outside the markers is never touched — with one exception discovered 2026-07-22: Codex re-serializes `~/.codex/config.toml` and drops every comment line, so the markers vanish while the managed keys survive as bare top-level lines; re-inserting the block beside such a survivor would duplicate a top-level key (invalid TOML — Codex would reject its whole config). The upsert therefore removes bare top-level copies of managed lines before re-inserting. Expect this drift to recur whenever Codex rewrites its config; re-running `--write` is the intended remedy. |
