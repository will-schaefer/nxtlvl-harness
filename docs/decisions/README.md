...
| [028](ADR-028-portable-sot-per-cli-supplements.md) | Multi-CLI config: `CLAUDE.md` (project + global) authored as a portable shared SoT; emitted instruction files are per-CLI supplements, never filtered copies; the compiler emits only the mechanical residue with a per-CLI verification step | Accepted |
