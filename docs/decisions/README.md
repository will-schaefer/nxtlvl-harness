...
| [036](ADR-036-repo-identity-nxtlvl-harness-only.md) | Narrow to a strict nxtlvl-harness identity: one published plugin (`nxtlvl`), `config/claude/` stays (harness's own Claude config + symlink source), terminal/editor dotfiles externalized to `will-schaefer/dotfiles`, `agent-dev` demoted to dormant-but-tracked (mirrors ADR-002), remote renamed `developer-config` → `nxtlvl-harness`, local dir unchanged — cross-links [001](ADR-001-plugin-local-marketplace-packaging.md), [002](ADR-002-ecc-dormant-reference-backstop.md) — "local dir stays `~/Developer`" clause superseded by [037](ADR-037-developer-parent-folder-harness-to-nxtlvl-subdir.md) | Accepted (amended 2026-06-25) |
| [037](ADR-037-developer-parent-folder-harness-to-nxtlvl-subdir.md) | Reframe `~/Developer` as a plain parent workspace folder; relocate the harness repo intact (full history) to `~/Developer/nxtlvl/`; de-nest `llm-wiki` to sibling; re-point symlinks, memory slug, and marketplace path — reverses ADR-036's "local dir unchanged" clause; all other ADR-036 decisions stand — cross-links [036](ADR-036-repo-identity-nxtlvl-harness-only.md), [001](ADR-001-plugin-local-marketplace-packaging.md) | Accepted (impl. deferred) |
